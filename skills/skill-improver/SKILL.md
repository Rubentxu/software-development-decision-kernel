...
- Use the XDG skill registry path supplied by SDDK as an index of skill names,
  triggers, scopes, and exact paths; do not expect generated summaries.
...
2. Read the supplied XDG skill registry; use listed paths to select skills. If
   missing, scan known installed skill directories for `*/SKILL.md`.
...
5. In apply mode, edit only safe issues, preserve content, create supporting
   files when needed, then refresh the `skill-registry` cache.
