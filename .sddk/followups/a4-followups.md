...
| `FU-A4-3-CONSTRAINT-BINDING` | Pre-A4-4b scope review (2026-09-16) | open (P1, must close before A4-5) | `software_alignment::reduce_alignment` still binds `ExplicitConstraint` to observations via string-matching on identifiers: `subject.contains(contract_ref)` and `canonical_tag.contains(contract_ref)` over textual forms of the references. That is binding-by-string and is the same `false-clean`-adjacent shape A4-2/DebVerify had to correct on the evidence side. Belongs to the A4-3 / Alignment integration boundary (A4-5 territory), not to the A4-4b abstraction budget. NOT FIXED HERE. Disposition documented in `docs/architecture/README.md` Checkpoint (A4-4b open, 2026-09-16). |
