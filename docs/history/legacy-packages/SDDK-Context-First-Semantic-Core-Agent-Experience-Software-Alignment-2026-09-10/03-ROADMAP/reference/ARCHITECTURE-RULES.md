...
## Software Alignment fitness rules

- Alignment cannot depend on concrete CogniCode/Chronos crates.
- Alignment cannot call side-effect capabilities.
- Generic lens heuristic cannot produce ContractViolation without explicit contract reference.
- Agent-facing Alignment suggestions must be labelled advisory.
- Verify cannot full-scan by default.
- DebVerify must inspect outside recent Git delta.
- KMT cannot replace SemanticGraph and vice versa.
- Workbook/chart/table state is rebuildable projection only.
- Missing provider evidence cannot be converted to PASS.
- New lens must identify universal concern it interprets and existing abstraction it reuses.
