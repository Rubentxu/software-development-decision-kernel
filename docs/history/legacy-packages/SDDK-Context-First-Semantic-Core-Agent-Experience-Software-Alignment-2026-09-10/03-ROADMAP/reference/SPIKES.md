...
## Software Alignment spikes

- **SA-SP1 Paradigm neutrality:** run same representative code through universal + OO + Functional lenses; verify no lens is silently normative.
- **KMT-SP1 Fingerprint sensitivity:** comments, rename, import, signature, body, generated code, analyzer version.
- **KMT-SP2 False-stale benchmark:** measure over/under invalidation on a medium fixture.
- **LLM-SP1 Progressive disclosure:** compare source-full vs card+delta vs card+provider evidence for token/cost/finding quality.
- **CG-SP1 CogniCode RPC:** cold/warm AnalyzeDelta latency, payload size and cache reuse.
- **CH-SP1 Chronos scenarios:** capture/query cost and BehaviorFingerprint stability.
- **WB-SP1 Projection storage:** generic tables vs specialized materialized views; choose lowest complexity satisfying UAT.
