...
## Software Alignment rules

- Consult Knowledge Cards before opening raw source when task scope permits.
- Treat Alignment suggestions as advisory, not mandatory.
- Preserve accepted tradeoffs/waivers until their assumptions or revisit triggers change.
- In Verify, respect changed/impact scope; in DebVerify, evaluate whole-project baseline progressively.
- Never report unavailable provider evidence as PASS.
- Update knowledge with typed dispositions (`CONFIRM/MODIFY/RETRACT/SUPERSEDE/STALE/UNKNOWN/NEW`).
