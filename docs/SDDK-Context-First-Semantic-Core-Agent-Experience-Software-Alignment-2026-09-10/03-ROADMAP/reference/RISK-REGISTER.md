...
## Software Alignment risks

| Risk | Severity | Mitigation |
|---|---|---|
| heuristic becomes accidental policy | critical | advisory BC + ContractViolation requires explicit contract |
| paradigm bias | high | universal concerns + selectable lenses |
| knowledge explosion | high | bounded cards, projection compaction, CAS refs |
| KMT over-invalidation | high | fingerprint vector + semantic overlay + benchmarks |
| provider coupling | high | SDDK-owned ports + capability negotiation |
| provider absence hides gaps | high | NOT_EVALUATED/EvidenceGap semantics |
| LLM churn | high | typed dispositions + previous assertion input |
| metrics gaming | medium | no universal score; attention-only composites |
