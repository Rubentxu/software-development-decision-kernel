...
| **A4-CLOSEOUT** | Cross-cutting A4 audit (042→047, follow-ups, single authorities, anti-encroachment, A5 readiness) — **not** another feature. Budget: AUDIT/RECONCILIATION/CERTIFICATION ONLY. | — | **CLOSED — v1.169.68** (release tag `v1.169.68` → SHA `3bad25275212f77c3d0d4d664f4d49293aa779c9`; A4 Milestone Receipt `docs/architecture/receipts/A4-MILESTONE-RECEIPT.md`; 19-test corpus `crates/sddk-engine/tests/a4_closeout_milestone_audit.rs`; final disposition **`A4_CERTIFIED`**; PublicReleaseGate PASS. **A4 CLOSED / CERTIFIED — A5-PLAN NEXT (not auto-opened).**) |
| **A4** | **Milestone CLOSED / CERTIFIED** — arch-spec-042→047 all `implemented`; single authorities preserved; no unresolved A4 blocker; deferred debt explicitly belongs to A5. | 042–047 | **CLOSED / CERTIFIED — v1.169.68** (`A4_CERTIFIED`; not `BASE_PRODUCTION_READY` — that is A5) |
| **A5-PLAN** | A5 planning (planning only; **no** `BASE_PRODUCTION_READY`): readiness contract, risk register, debt disposition, workstream DAG, UAT + falsification matrices, release/certification protocol, deferred-post-BASE list. Detail under `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/`. | — | **CLOSED — planning** (cycle `p-63676b11dc0ef88f/a5-plan-base-production-ready`; deliverables `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/`; handoff `docs/history/handoffs/all-handoffs/HANDOFF-2026-09-17-a5-plan-base-production-ready.md`; planning-only, **no production release published**) |
| **A5-1** | Release / distribution / version governance — semantic push/release admission (subject is not authority; real version bump or non-empty docs-only range), rollback UAT, distribution integrity | `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/` | **CLOSED — v1.169.71** (release tag `v1.169.71` → SHA `e3fc3a09259182478c5f508bc4d51ceb958260b5`; receipt `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/A5-1-RECEIPT.md`; matrices 20+7 green; G7/G8/G15/G16 evidenced; PublicReleaseGate PASS; `INC-A5-PUSH-RELEASE-MARKER-FRICTION` + `INC-A4-RELEASE-VERSION-DRIFT` CLOSED. A5-2 NEXT — not auto-opened.) |
| **A5-2** | Durability / rebuild / recovery | `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/` | **NEXT** (unblocked by A5-1; not auto-opened) |
| **A5-3** | Concurrency / CAS / authority side-effect races | `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/` | blocked_by A5-2 |
| **A5-4** | Compatibility / deprecation / lints / operator UX | `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/` | blocked_by A5-1 |
| **A5-5** | Reliability / clean-machine / security / operational UAT | `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/` | **CLOSED** — v1.169.87 (`5ad25d1`, A5-5-CLEAN-MACHINE-SWEEP-RECEIPT.md, UAT-1 10/10 on isolated podman container) |
| **A5-C** | `BASE_PRODUCTION_READY` certification (audit + milestone receipt) | `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/` | **CERTIFIED** — v1.169.88 (`add896d`, A5-C-BASE-PRODUCTION-READY-CERTIFICATION.md; G0..G15 GREEN, G11 NOT VERIFIED with R14 accepted, 11 install.sh defects classified with zero RELEASE_BLOCKER, public install path validated without harness workarounds) |
...
collision with the `docs/history/legacy-packages/sddk-decision-kernel-architecture/03-adrs/` range
...
- **Manifest SHA-256**: see
  `docs/history/legacy-packages/SDDK-Semantic-Core-Agent-Experience-Consolidation-2026-09-09/MANIFEST.sha256`
...
- `docs/history/legacy-packages/SDDK-Production-Readiness-Alignment-2026-09-14/05-CHRONOS-HANDOFF.md`
...
- `docs/history/handoffs/all-handoffs/ROADMAP-COMPLETION-RECEIPT-2026-09-18-post-A5-4b.md`
...
For the live roadmap, read `docs/history/legacy-packages/architecture-a5-a6/architecture-a5/A5-CURRENT-ROADMAP.md`.
