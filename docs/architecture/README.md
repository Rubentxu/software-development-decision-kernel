...
| **M8** | WHY engine + causal explanation | partial — M8.0 `sddk dev graph list/edges/projection` operator surface for the H9 Active Graph & Cockpit engine (v1.166.5) + M8.1 `sddk dev graph why` causal queries (why/debt-why/decision-why) for the WHY engine (v1.166.6); engine modules shipped (`active_graph.rs` 588 LoC + `why_queries.rs` 574 LoC) but lacked CLI; cockpit overview/journal/timeline remains M8.2+ |
