...
| **A4-4M** | Convergence of existing `paradigm_lens` (OO/FP/ADT/DSL) onto generic `Alignment` — zero feature | `arch-spec-046` migration | **closed** (release SHA pending the release publish; M0 Migration Proof gate passed — `docs/architecture/a4-4m-migration-matrix.md`; production lenses via `AlignmentLensRegistry`; `status_from_polarities` + inferred path deleted) |
| **A4-4C** | Receipt/UAT for `arch-spec-046` milestone closure | — | **CURRENT** (was blocked_by A4-4M; does NOT auto-open — STOP rule in cycle spec) |
