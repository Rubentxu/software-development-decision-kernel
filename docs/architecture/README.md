...
|---|---|---|---|
| **A4-0** | Evidence → observes → SoftwareRelation (provenance foundation) | `arch-spec-042` | implemented — v1.168.35 |
| **A4-1** | Generic Verify kernel (`verify_kernel` + AC4 adapter) | `arch-spec-043` | implemented — v1.169.46 |
| **A4-2** | Generic DebVerify kernel (`debverify_kernel` + AC5 adapter + ObservationContradiction) | `arch-spec-044` | implemented — v1.169.47 |
| **A4-2M** | AC4/AC5 convergence onto generic kernels (single execution surface) | `arch-spec-043/044` | implemented — v1.169.48 |
| **A4-3** | Software Alignment domain (closed states + closed findings; pure reducer; no authority) | `arch-spec-045` | in progress (this cycle) |
| **A4-4** | Intent + UniversalConcern + AlignmentLens (poly-paradigm policy preserved) | `arch-spec-046` | pending |
| **A4-5** | Intelligence loop wiring: Verify + DebVerify + Alignment → AdvisoryContext + WHY/WHY-NOT | `arch-spec-047` | pending |
| **A4-CLOSEOUT** | A4 acceptance gate + A5 readiness check | — | pending |
| **A5** | `BASE_PRODUCTION_READY` — release receipts, hardening programme close | — | pending |
