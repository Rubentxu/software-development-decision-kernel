...
| `PR-GAP-004` | Revision substrate vs Decision Memory | `PARTIAL` | Classify every OID/ref/CAS/ancestry primitive as shared, decision-semantic, compatibility, or redundant. Semantically identical generic invariants have one implementation. Narrow differences require ADR. | Base |
| `PR-GAP-005` | SQLite schema ownership | `PARTIAL` | Define one migration/schema owner. Stores consume an already-governed schema/connection contract rather than independently evolving shared tables. Add fresh DB, migrated DB, FK and crash/reopen fixtures. See ARCH-SPEC-020. | Base |
| `PR-GAP-006` | AuthorityEngine cutover | `PARTIAL` | Enumerate governed effects; route all through one AuthorityEngine decision path; complete the `LowMedium → All` rollout or replace that staging mechanism with an equally explicit cutover; prove zero bypass. | Base |
...
| `PR-GAP-008` | Public legacy compatibility surface | `PARTIAL` | Active API stops glob-exporting legacy semantics. Required historical decode paths move behind an explicit compatibility namespace/allowlist with owner, fixture and removal trigger. | Base |
