...
| `PR-GAP-002` | Universal Evidence convergence | `PASS_WITH_COMPAT` | No production write may use legacy `PlanningEvidenceKind` as authority. Legacy taxonomy may survive only as decode compatibility with removal trigger. **Evidence (2026-09-14):** `evidence_kind_v1` lint is `default = deny` (blocking) with 0 hits after bounded `exclude_paths` (definition/mapping/decoder/corpus); `sddk dev lint deprecated-patterns` enforces it; E1 authors evidence via `resolve_planning_evidence_relation` / `EvidenceAttachmentRecord::from_universal_relation` (`CoreRelationKind`). Compat: legacy enum is read-only decode with removal trigger (C2 window close). | Base |
...
| `PR-GAP-006` | AuthorityEngine cutover | `PASS` | Enumerate governed effects; route all through one AuthorityEngine decision path; complete the `LowMedium → All` rollout or replace that staging mechanism with an equally explicit cutover; prove zero bypass. **Evidence (2026-09-14, B+ / ADR-0111):** the engine now raises `RequireApproval` only from explicit `PolicySnapshot.approval_required_for` (no implicit `risk_band == High`); `bridge::default_approval_required_for` lists only dangerous actions (`CycleSupersede`, `CliRelease`, `CliShip`, `PackInstall`). `ENFORCEMENT_STAGE = All`; the advisory path is unreachable in production. Evidence: engine + action×surface×actor matrix tests, grant→re-admit, `zero_bypass_every_surface_blocks_require_approval`, the 16 formerly-broken CLI workflows green, `cli_approval_loop_e2e` green. ADR: `docs/architecture/adrs/ADR-0111-EXPLICIT-APPROVAL-POLICY.md`. | Base |
| `PR-GAP-007` | Command registry source of truth | `PASS_WITH_COMPAT` | Eliminate authoritative handwritten duplication between the CLI command model and `CommandSpec`. Help, examples, agent surface and allowed-command sets derive from one typed substrate. **Evidence (2026-09-14):** `15-A0-PR-GAP-007-COMMAND-REGISTRY-CROSSWALK.md`. `CommandSpec` is the single typed source for introspection/typed help, agent surface, examples and allowed-command sets. Comp: the clap `Command` enum remains the runtime argv parser, coupled by the mandated drift test `clap_surface_and_command_specs_are_in_sync` (owner `sddk-cli`; removal trigger: spec table generated from clap/runtime introspection). | Base |
...
| `PR-GAP-009` | Dead/staged code and dependencies | `PASS` | Run dead dependency/code scans; remove real dead dependencies. Every surviving `allow(dead_code)`, deprecated path or staged branch has owner + reason + exit trigger. **Evidence (2026-09-14):** removed unused deps (`anyhow` in domain/gateway/vault; `serde`/`serde_json`/`thiserror` in pack-uat) and dead code (`MIGRATION_10` const, `telemetry::SCHEMA_V1` + `upsert_uat_result` wrapper). All 83 `#[allow(dead_code)]` sites carry owner/reason/exit; all 3 `#[deprecated]` paths carry owner + removal trigger; the 34 `#[allow(unused)]` audit-guard consts are documented. `cargo check --workspace --all-targets`, `clippy --workspace --all-targets -D warnings`, `cargo fmt --check` green. | Base |
| `PR-GAP-010` | Documentation authority drift | `PASS` | Reconcile 09/09 baseline, C0→C7 closeout and 10/09 roadmap into one documented succession. No implemented normative spec remains `proposed` without reason; no historical package claims competing authority. **Evidence (2026-09-14):** `docs/architecture/specs/README.md` now records one explicit succession chain (09/09 → C7 → 10/09 → 14/09 → receipts) and a per-spec crosswalk giving the reason each of arch-spec-001..031 stays `proposed` (baseline awaits the C7 receipt; convergence/agentic await their milestones). Historical packages remain listed as superseded in `docs/architecture/README.md`. | Base |
| `PR-GAP-011` | C7 conformance receipt | `PARTIAL` | SPEC-001..018 all PASS/PASS_WITH_COMPAT, UAT-01..22 green, clean + migrated + recovery + projection-rebuild runs green, commit SHA recorded. **Progress (2026-09-14):** `docs/SDDK-Context-First-Semantic-Core-Agent-Experience-Software-Alignment-2026-09-10/08-BASELINE-CONFORMANCE-09-09/09-09-CONFORMANCE-RECEIPT.md` produced with SPEC/UAT/M9/recovery matrices at commit `368a9d2`. Remaining MUST before PASS: PR-GAP-002 closeout (legacy `PlanningEvidenceKind` guard), dedicated negative fixtures for UAT-10/UAT-20 and the obsolete-prompt M9 row, and SPEC-012/SPEC-013 precedence/asset-closure fixtures. | Base |
