...
This module extends `apply.md`; it has no standalone transition. The parent
phase's ledger contract covers strict-TDD implementation evidence.

Transition reference (inherited from parent apply phase):
```
Transition:   phase.build.complete
Matrix row:   lifecycle.cycle.transition.apply
Artifact:     {cycle_artifacts_dir}/apply-progress.md
On failure:   blocked — runtime remains OPEN/build; strict-TDD cycle evidence gates unsatisfied
```

Full procedure (from `cli-usage-contract.md#matrix`):
1. `sddk cycle status --root . --scope . --cycle {cycle_id} --format json` → confirm phase is `build`.
2. Build `{evidence_json}` with TDD cycle evidence table path/SHA-256, RED/GREEN/TRIANGULATE/REFACTOR
   results per task, safety-net results, and one result per assertion-quality gate.
   Set `{outcome}` to `passed` only when every behavior-bearing task has RED→GREEN evidence.
3. `sddk cycle evaluate-gate --root . --scope . --cycle {cycle_id}
   --transition phase.build.complete --gate tdd-evidence-complete
...
On failure: blocked — strict-TDD cycle gates are mandatory. A failed CLI invocation
