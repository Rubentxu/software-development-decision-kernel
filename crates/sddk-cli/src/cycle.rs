...
pub(crate) struct RuntimeContext {
    ...
    /// AuthorityEngineRunner facade (SPEC-M5). Pre-gates every governed
    /// CLI write before delegating to the engine.
    #[allow(dead_code)]
    pub(crate) authority_runner: sddk_engine::authority_engine::AuthorityEngineRunner,
...
fn run_cycle_lock_release(
    ...
    let result = (|| -> anyhow::Result<CycleLockReleaseOutput> {
        ...
        let actor = args
            ...
            .or_else(|| environment.user.clone())
            .unwrap_or_else(|| "sddk-cli".into());
        let released = context.storage.release_lease_with_event(
...
mod tests {
    use super::*;
    use sddk_domain::{
        CyclePath, CycleStatus, Ledger, Phase, ProjectId, ProjectRecord, WorkspaceRecord,
