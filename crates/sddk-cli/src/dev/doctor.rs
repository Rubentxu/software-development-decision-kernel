    ...
    if responsibilities_yaml_path.is_file() {
        match std::fs::read_to_string(&responsibilities_yaml_path) {
            Ok(text) => {
                ...
                // M7.1: Command Registry authority markers.
                for marker in m7_1_command_registry_alignment_checks(&text) {
                    push_marker(&mut checks, &marker);
                }
                ...
                // M7.2: Surface Integration Layer marker.
                for marker in m7_2_surface_integration_alignment_checks(&text) {
                    push_marker(&mut checks, &marker);
                }
                // M7.4: Full AgentProfile model marker.
                for marker in m7_4_agent_profile_alignment_checks(&text) {
                    push_marker(&mut checks, &marker);
                }
                ...
                // M8.3: Cockpit-observability surface marker.
                for marker in m8_3_cockpit_observability_surface_alignment_checks(&text) {
                    push_marker(&mut checks, &marker);
                }
