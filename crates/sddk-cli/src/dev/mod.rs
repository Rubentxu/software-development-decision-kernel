...
mod editor_adapters;
mod entropy;
pub mod framework_bundle_ticket;
mod framework_check;
pub mod github_releases_ticket;
pub(super) mod graph;
mod install;
mod link;
pub(super) mod lint {
...
pub(super) fn run_dev(command: DevCommand, environment: &CliEnvironment) -> CommandOutput {
    match command {
        ...
        DevCommand::Cockpit(args) => match args.command {
            ...
            crate::dev::cockpit::CockpitCommand::DiffWatch(watch_args) => {
                self::cockpit::run_dev_cockpit_diff_watch(watch_args, environment)
            }
            crate::dev::cockpit::CockpitCommand::Digest(digest_args) => {
                self::cockpit::run_dev_cockpit_digest(digest_args, environment)
            }
        },
