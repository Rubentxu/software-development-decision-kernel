...
use crate::dev::framework_bundle_ticket::{
    FrameworkBundleTicketError, with_framework_bundle_ticket,
};
...
// using sddk_engine::authority_engine::ActorKind below
use sha2::Digest;

pub(super) fn run_dev_install(args: super::InstallArgs) -> CommandOutput {
    let format = args.format;
    // A6-1 — wrap the `framework_bundle` install body inside the
    // `AdmissionTicketBus` wrapper. The R4-A `auth.validate(...)` short-circuits
    // above this closure (preserved); the wrapper adds the R4-B re-validation
    // boundary. If the consume refuses (PolicyChanged / FenceExpired /
    // TicketAlreadyConsumed) the body never runs and no prefix mutation happens.
    //
    // Pattern: ADR-0131 §Migration Pattern.
    let actor_for_ticket = args
        .actor
        .clone()
        .or_else(|| std::env::var("SDDK_ACTOR").ok())
        .unwrap_or_else(|| "system".into());
    // The ticket wrapper requires the engine-flavoured ActorKind. The
    // CLI surfaces are all System actor at runtime, so a direct construction
    // is enough; the typed-auth-validate path still gates the rest.
    let actor_kind_for_ticket = match actor_for_ticket.as_str() {
        a if a.starts_with("agent:") => sddk_engine::authority_engine::ActorKind::Agent {
            profile_id: a.to_string(),
            profile_version: 1,
        },
        a if a.starts_with("orchestrator:") => {
            sddk_engine::authority_engine::ActorKind::Orchestrator {
                session_id: a.to_string(),
            }
        }
        _ => sddk_engine::authority_engine::ActorKind::System {
            service: "sddk-cli".to_string(),
        },
    };

    let result = with_framework_bundle_ticket(
        sddk_engine::authority_engine::Actor {
            kind: actor_kind_for_ticket,
            capabilities: vec!["cli.execute".to_string()],
            lease: None,
        },
        "framework_bundle",
        || -> anyhow::Result<super::InstallReceipt> {
            ...
            let ends_with_bin =
                args.prefix.file_name().and_then(|name| name.to_str()) == Some("bin");
    ...
    // Map the helper's typed `FrameworkBundleTicketError` to `anyhow::Error`
    // so `render_result` receives a uniform `Result<T, anyhow::Error>`.
    let result: anyhow::Result<super::InstallReceipt> = match result {
        Ok(receipt) => Ok(receipt),
        Err(FrameworkBundleTicketError::Denied(reason)) => Err(anyhow::anyhow!(
            "framework_bundle admission denied: {reason}"
        )),
        Err(FrameworkBundleTicketError::Ticket(reason)) => {
            Err(anyhow::anyhow!("framework_bundle ticket refused: {reason}"))
        }
        Err(FrameworkBundleTicketError::Effect(err)) => Err(err),
    };
