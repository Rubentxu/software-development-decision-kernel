...
fn cli_walks_cycle_with_fencing_and_rebuilds_state() {
    ...
    // events. After B+ (ADR-0111) the routine `gate_receipts`/`transition_records`
    // admissions are `Allow`, so no `authority.admission.decided` event is
    // recorded for them (only blocking/approval decisions are recorded).
    // 3 + 3 = 6.
...
/// S2 (REMOVED 2026-09-18, A5-ITD OBSOLETE):
/// phase.build.remediate is rejected when source state is REMEDIATING/verify.
///
/// Disposition: OBSOLETE.
/// Reason: `REMEDIATING/verify` is a state-machine phantom — the workflow
/// only knows `REMEDIATING/build` (`release.recover` is the only entry
/// into the REMEDIATING state). The test's preconditions can never be
/// satisfied against the canonical workflow.
///
/// Live coverage of the property the test wanted to prove
/// (source-state mismatch is rejected at the source-state check):
///   `cli_phase_build_remediate_transitions_to_open_build` (S1, green)
///   — exercises the exact source-state check on every successful
    ...
    while let Some(dir) = stack.pop() {
        if let Ok(entries) = std::fs::read_dir(&dir) {
            for entry in entries.flatten() {
                ...
                } else if ft.is_some() && (entry.file_name().to_str() == Some("ledger.sqlite")) {
