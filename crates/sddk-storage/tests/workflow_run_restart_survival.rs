...
fn run_survives_restart_with_equivalent_identity_and_provenance() {
    ...
    {
        let mut store =
            SqliteGraphStore::open(&ledger_path).expect("failed to open store on temp dir");
    ...
    {
        let store =
            SqliteGraphStore::open(&ledger_path).expect("failed to reopen store on temp dir");
        ...
        assert!(
            loaded.is_some(),
            "load_run should find the run after restart"
        );
        let loaded = loaded.unwrap();
        assert_eq!(loaded.run_id, run_id, "run_id must match");
        assert_eq!(
            loaded.template_ref.id, "test.template",
            "template_ref.id must match"
        );
        assert_eq!(
            loaded.template_ref.version, "1.0.0",
            "template_ref.version must match"
        );
        assert_eq!(
            loaded.correlation_id, correlation_id,
        ...
        assert_eq!(
            latest,
            Some(WorkflowRunState::Running),
            "latest state should still be running after restart"
        );
        ...
        let loaded_rev = store
            .load_revision(&run_id, &fresh_revision.revision_id)
            .expect("load_revision failed");
        assert!(
            loaded_rev.is_some(),
            "revision should be loadable after restart"
        );
...
fn restart_survives_with_deterministic_run_id() {
    ...
    {
        let mut store = SqliteGraphStore::open(&ledger_path).expect("failed to open store");
        store
            .record_run(&run, &revision)
            .expect("record_run failed");
    }
    ...
    {
        let store = SqliteGraphStore::open(&ledger_path).expect("failed to reopen store");
        let loaded = store.load_run(&rederived_run_id).expect("load_run failed");
        assert!(
            loaded.is_some(),
            "re-derived run_id should find the persisted run"
        );
        assert_eq!(
